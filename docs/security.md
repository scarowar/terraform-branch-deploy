...
| Plan intent binding | Every plan run records an intent artifact before Terraform runs. Apply resolves the plan only through the newest intent record, so a superseded plan is never applied, and a failed or in-flight latest plan attempt blocks apply. |
...
Saved plan files and metadata persist across the plan and apply runs as GitHub Actions workflow artifacts, which are immutable once uploaded. Each plan run also records an intent artifact before Terraform runs; apply resolves the newest intent (an explicit numeric sort by workflow run, never the API's list order) and requires the exact plan artifact that intent names — if the most recent plan attempt failed, apply refuses rather than falling back to an older plan. The artifact name includes the saved plan params hash, and apply refuses a restored plan if the name and metadata disagree. Artifact extraction rejects unsafe archive member paths and decompression-bomb sizes. Plan artifacts expire after the `plan-retention-days` input (default 7 days, capped by the repository's artifact retention setting); an expired plan means re-running `.plan`. A plan run fails loudly if the artifact upload fails — a plan that cannot be applied later is never reported as successful. Plan files can embed sensitive resolved values, so keep retention short and restrict who can read repository artifacts.
